    ...
    def train(self, t_end=None):
        ...
        for epoch in range(self.epoch, self.max_epoch + 1):
            ...
            for batch in self.train_loader:
                ...
                img = batch['rgb_norm'].to(self.device)
                sunny = batch['sunny_norm'].to(self.device)
                with torch.no_grad():
                    lnt, logvar = self.encode_rgb(img)

                # Forward sunny and weather
                pred_latent = self.model(img, lnt)
                reconstruct_img = self.decode_rgb(pred_latent)
                loss = self.smooth_l1_loss(reconstruct_img, sunny)
